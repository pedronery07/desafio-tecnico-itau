# Uso de IA

Quais ferramentas de IA foram usadas, para quê, e algum ponto em que a IA levou para o caminho errado e isso foi percebido.

Usei Claude Code (Sonnet - esforço médio) para montar a estrutura inicial de pastas/arquivos do repositório e como par de programação ao longo da implementação.

Pontos em que a IA errou e eu percebi:

## Nível 1
- Ao montar os primeiros gráficos da análise exploratória, ela plotou o histograma de `valor` usando os dados brutos, misturando operações em BRL e USD sem converter — um gráfico com unidades incoerentes. Pedi a correção e ela normalizou os valores para BRL antes de plotar.
- Na comparação de custo entre os dois prompts do Nível 1, ela escreveu a análise olhando só `tokens_total`, o que sugeria que os dois prompts custavam praticamente o mesmo. Percebi que isso escondia o fato de `tokens_prompt` (o contrato de saída mais rígido do prompt v2) e `tokens_resposta` (a resposta mais livre e longa do prompt v1) puxarem em direções opostas e quase se cancelarem no total — pedi para decompor a análise nesses dois componentes em vez de comparar só o total.

## Nível 2
- Na Parte A, usei a IA para refatorar a lógica de limpeza/regras do notebook do Nível 1 em `nivel_2/regras.py` (funções reutilizáveis) e escrever `nivel_2/parte_a.py`, que roda o pipeline na base maior, gera os gráficos de análise exploratória e o ranking dos 10 clientes mais sinalizados.
- Na Parte B, usei a IA para implementar `tools.py` e o loop de function calling manual em `agente.py`. Testei os dois casos reais (fracionamento e valor atípico) e validei que o agente decide sozinho quais ferramentas chamar em cada caso.
- Na Parte C, pedi para implementar cache em disco (ideia minha, mas sugerida também pelo enunciado). Ao rodar o lote pela primeira vez, o script quebrou no meio por rate limit (429) da camada gratuita. A IA identificou a causa pelo próprio erro da API e implementou retry com backoff sem eu precisar apontar o problema; rodei de novo e completou os 10 clientes.
- Na Parte D (confronto), a IA rodou o primeiro confronto e obteve só 30% de concordância entre regra e agente, com um padrão suspeito (todos os clientes de "valor atípico" caindo pra risco baixo). Em vez de aceitar isso como "o agente discorda com boa justificativa", ela mesma investigou a fundo e descobriu que `historico_cliente` não dava ao agente a data exata do fracionamento nem a magnitude do valor atípico. Ou seja, o agente estava decidindo com informação incompleta, e num caso (`CLI-003`) até investigou a data errada. Ela abriu a possibilidade de apenas documentarmos esse problema e seguir em frente ou corrigir isso agora mesmo. Optei por tratar isso pois achei o problema muito grave para ser ignorado; depois da correção a concordância subiu para 80%, com divergências bem mais fundamentadas.
- Após a correção, fiquei na dúvida se de fato tomei a decisão correta. Dar ao agente o contexto exato de como a regra determinística foi feita (que o próprio enunciado classifica como propositalmente simples) não introduz um viés na tomada de decisão do próprio agente? Fiquei com esse questionamento depois que percebi o aumento drástico da taxa de concordância. Por outro lado, sem o contexto exato, o agente realmente estava chamando ferramentas incorretamente.

## Nível 3
- Pedi para implementar a Trilha A (multiagente) com Triador/Investigador/Redator, estado compartilhado e condição de parada. Escolhi os 3 primeiros clientes de teste (`CLI-003`, `CLI-023`, `CLI-028`). A IA percebeu sozinha que nenhum dos três parou no Triador, procurou entre os outros clientes da base (fora do top10) o sinal mais fraco possível e sugeriu `CLI-008` como 4º caso. Esse sim fez o Triador arquivar direto, provando a condição de parada de verdade em vez de eu forçar um resultado.
